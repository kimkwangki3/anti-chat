...
const BoardPage = () => {
    ...
    return (
        <div className="h-full bg-[#0a0a0f] text-[#e8e8f0] p-8 overflow-y-auto custom-scrollbar pt-safe">
            <div className="max-w-6xl mx-auto">
                <header className="flex justify-between items-center mb-12">
                    <div>
                        <h1 className="text-4xl font-bold font-['Bebas_Neue'] tracking-wider mb-2 uppercase italic text-white">{currentChannel?.name || 'CHANNEL'} <span className="text-[#4f6ef7]">BOARD</span></h1>
                        <p className="text-[#6b6b8a] text-[10px] font-mono tracking-[0.3em] uppercase ml-1">채널 내부 전용 게시판</p>
                    </div>

                    {isAdmin && (
                        <button
                            onClick={() => setShowForm(!showForm)}
                            className={`px-8 py-3 rounded-xl font-bold text-xs transition-all border ${showForm ? 'bg-red-500/10 border-red-500/20 text-red-500' : 'bg-[#4f6ef7] border-[#4f6ef7] text-white shadow-lg shadow-[#4f6ef7]/20 hover:scale-105'}`}
                        >
                            {showForm ? '취소' : '+ 새로운 글 작성'}
                        </button>
                    )}
                </header>
