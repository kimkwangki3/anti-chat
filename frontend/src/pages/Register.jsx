...
const Register = () => {
    ...
    return (
        <div className="min-h-screen bg-[#1a1a24] flex items-center justify-center p-6 relative overflow-hidden">
            {/* Decorative Orbs */}
            <div className="absolute top-[-10%] right-[-10%] w-[40%] h-[40%] bg-[#FF9500]/10 rounded-full blur-[120px]"></div>
            <div className="absolute bottom-[-10%] left-[-10%] w-[50%] h-[50%] bg-[#FF5E00]/5 rounded-full blur-[120px]"></div>
                ...
                <div className="bg-[#23232f] border border-white/5 rounded-[2.5rem] p-10 md:p-12 shadow-[0_20px_50px_rgba(0,0,0,0.3)] backdrop-blur-2xl">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        {formData.role === 'member' ? (
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    type="text"
                                    required
                                    placeholder="?¥ÏòÅ??Ï±ÑÎÑê ?¥Î¶Ñ???ÖÎ†•?òÏÑ∏??
                                    className="w-full bg-[#1a1a24] border border-white/5 rounded-2xl px-6 py-4 text-white text-sm focus:outline-none focus:border-[#FF9500]/50 transition-all placeholder:text-[#3a3a4a] font-medium"
                                    value={channelName}
                                ...
                                {channelName && (
                                    <p className="text-[11px] text-[#FF9500] ml-4 font-mono">
                                        ?Ä?îÎ™Ö: <span className="font-bold">{channelName} Í¥ÄÎ¶¨Ïûê</span>
                                    </p>
                                )}
                            </div>
                        )}
                        ...
                        <button
                            type="submit"
                            disabled={isLoading}
                            className="w-full h-16 orange-gradient hover:scale-[1.02] active:scale-95 text-white font-black py-4 rounded-3xl shadow-2xl shadow-[#FF9500]/20 transition-all text-xs tracking-[0.3em] uppercase mt-4 flex items-center justify-center"
                        >
                            {isLoading ? (
                    ...
                    <div className="mt-10 text-center">
                        <p className="text-[11px] text-[#6b6b8a] font-bold uppercase tracking-widest">
                            Already Have an Account? <Link to="/login" className="text-[#FF9500] hover:text-[#FFB347] transition-colors decoration-[#FF9500]/20 decoration-2 underline-offset-8 underline">Login Now</Link>
                        </p>
                    </div>
                </div>
            </div>
